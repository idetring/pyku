...
class LMK:
    ...
    """
    Linear Monge Kantorovich (LMK) for numpy arrays of size (nfeatures x
    nsamples)
    """
    ...
    def fit(self, *, np_cal=None, np_obs=None):
        ...
        Arguments:
            np_cal (:class:`numpy.ndarray`): The biased dataset with shape
                (nfeatures x nsamples).
            np_obs (:class:`numpy.ndarray`): The reference dataset with shape
                (nfeatures x nsamples).
        """
        ...
        cal = np_cal.astype(float_type)
        obs = np_obs.astype(float_type)

        # Get the number of features
        # --------------------------

        nfeatures = cal.shape[0]

        # Calculate intermediate matrices
        # -------------------------------

        sigma_v = np.cov(obs, dtype=float_type)
        sigma_u = np.cov(cal, dtype=float_type)

        sqrt_sigma_u = sqrtm(sigma_u)
        # sqrt_sigma_v = sqrtm(sigma_v)

        inv_sqrt_sigma_u = inv(sqrt_sigma_u)
        # inv_sqrt_sigma_v = inv(sqrt_sigma_v)

        # Determine means
        # ---------------

        obs_mean = np.empty(nfeatures, dtype=float_type)
        cal_mean = np.empty(nfeatures, dtype=float_type)

        for feat_idx in range(nfeatures):
            obs_mean[feat_idx] = np.mean(obs[feat_idx])
            cal_mean[feat_idx] = np.mean(cal[feat_idx])
        ...
        self.transfer = \
            inv_sqrt_sigma_u \
            @ sqrtm(sqrt_sigma_u @ sigma_v @ sqrt_sigma_u) \
            @ inv_sqrt_sigma_u
    ...
    def predict(self, *, np_mod=None):
        ...
        Arguments:
            ...
            np_mod (:class:`numpy.ndarray`): The biased dataset with shape
                (nfeatures x nsamples).

        Returns:
            :class:`numpy.ndarray`: The corrected data with shape
                (nfeatures x nsamples)
        """
        ...
        # We need to substract and add the mean for nsamples. The array shall
        # have shape (nfeature x nsamples)

        cal_mean = np.tile(self.cal_mean, (np_mod.shape[1], 1))
        obs_mean = np.tile(self.obs_mean, (np_mod.shape[1], 1))

        cal_mean = np.transpose(cal_mean)
        obs_mean = np.transpose(obs_mean)

        cor = self.transfer @ (np_mod - cal_mean) + obs_mean
        ...
        def fit_predict_each_pixel(
            ...
            for i in np.arange(block_mod[x_name].size):
                for j in np.arange(block_mod[y_name].size):
                    ...
                    elif self.bc_method_id.lower() in ['lmk']:

                        corrector = LMKCorrector(
                            ds_obs=block_obs_ij,
                            ds_cal=block_cal_ij,
                        )
...
class LMKCorrector(PDFCorrector):

    """
    Linear Multivariate Probability Distribution Corrector (LMK) for xarray
    DataSet

    The linear Monge-Kantorovitch colour mapping for example-based colour
    transfer, F. Pitie, A. Kokaram, 2007
    """

    def __init__(self, *, ds_cal=None, ds_obs=None):
        ...
        Arguments:
            ds_obs (:class:`xarray.Dataset`): Observation/Reference dataset
            ds_cal (:class:`xarray.Dataset`): Biased model dataset used for
                calibration
        """
        ...
        self.bc_method_id = 'lmk'
        self.bc_method = textwrap.dedent(
            """
        ...
        if 'time' in self.ds_cal.dims:
            cal_begin_yyyy = self.ds_cal.time.dt.year.min().item()
            cal_end_yyyy = self.ds_cal.time.dt.year.max().item()
            self.bc_period = f'{cal_begin_yyyy}-{cal_end_yyyy}'
        else:
            self.bc_period = 'Not applicable'
    ...
    def __str__(self):
        ...
        message = textwrap.dedent(
            ...
            bc_method_id: {self.bc_method_id}
            bc_period: {self.bc_period}
    ...
    def fit(self):
        ...
        if obs_varnames != cal_varnames:
            ...
            message = textwrap.dedent(
                f"""
                obs_varnames is {obs_varnames} should equal cal_varnames
                {cal_varnames}
                """
            )
        ...
        # Warn in case of low statistics
        # ------------------------------

        if len(nu_cal[0]) < 1000:
            message = f"Low statistics with {len(nu_cal[0])} data points."
            logger.info(message)

        # Determine the transfer function matrix
        # --------------------------------------

        if nu_cal.size != 0 or nu_obs.size != 0:
    ...
    def predict(self, ds_mod=None):
        ...
        Arguments:
            ds_mod (:class:`xarray.Dataset`): Biased model dataset
        ...
        mod_varnames = sorted(libmetadata.get_geodata_varnames(ds_mod))

        # Construct numpy arrays of size nfeatures x npixels
        # --------------------------------------------------

        np_mod = np.array([
            ds_mod[var].values.reshape(-1) for var in mod_varnames
        ])
        ...
        # If the array contains only NaNs, return only Nans
        # -------------------------------------------------
        ...
        if nu_mod.size == 0:
            ...
            original_shape = ds_mod[mod_varnames[0]].shape
        ...
        ds_cor.attrs['bc_method_id'] = self.bc_method_id
        ds_cor.attrs['bc_method'] = self.bc_method
        ds_cor.attrs['bc_period'] = self.bc_period
        ds_cor.attrs['creation_date'] = self.creation_date
    ...
    def get_transformation(self):
        ...
        """
        Get transformation as a dataframe
        ...
        names = sorted(libmetadata.get_geodata_varnames(self.ds_obs))
        df = pd.DataFrame(self.method.transfer, index=names, columns=names)
